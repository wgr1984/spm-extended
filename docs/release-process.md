...
- **Package dependency:** Users depend on `from: "0.2.1"` (or similar). They need a tag that matches that version.
...
1. If you specify a version (e.g. `mint run wgr1984/spm-extended@0.2.1` or `@main`), that ref is used.
...
- Users can depend on `.package(url: "...", from: "0.2.1")` (or exact version) and get that tag.
