...
- **SamplePackage/** – Minimal Swift package that depends on the plugin via `path: "../.."`, on **sample.DemoLib** from the local registry (for the outdated demo), and on **swift-numerics** from Git (pinned to 1.0.0 so outdated shows 1.1.1 available). Use it to try `registry metadata create`, `registry publish`, and `swift package outdated`.
